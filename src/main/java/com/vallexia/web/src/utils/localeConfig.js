...
export const ensureLocaleConfigLoaded = async () => {
  if (!configPromise) {
    configPromise = localeService.getLocaleConfig()
      .then((config) => {
...
export const getDefaultMeasurementSystemCode = () => getMeasurementSystems()[0]?.code || null
export const getDefaultFirstDayOfWeekCode = () => getFirstDayOfWeek()[0]?.code || null
export const getDefaultGoalTypeCode = () => getGoalTypes()[0]?.code || null
export const getDefaultSubscriptionStatusCode = () => getSubscriptionStatuses()[0]?.code || null
export const getDefaultMealTypeCodes = (count = 3) => getMealTypes().slice(0, count).map((item) => item.code)

export const getFormattingRuleForCountry = (countryCode) => {
  if (!countryCode) {
    return null
  }
  return getFormattingRules().find(
...
export const getTokensForDateFormat = (format) => {
  ...
  return getDateFormats().find((item) => item.format === format)?.tokens || null
}
