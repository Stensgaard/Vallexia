 ...
 */
const formatLanguageName = (code) => code?.charAt(0).toUpperCase() + code?.slice(1) || ''
...
let supportedLanguageCodes = new Set(fallbackLanguages.map(lang => lang.code))
...
const normalizeLanguages = (languages) => {
  ...
  return languages
    .map((lang) => {
      ...
      const code = typeof lang === 'string' ? lang : (lang.code || lang.locale || '')
      ...
      return {
        code: normalizedCode,
        name: (typeof lang === 'object' && lang.name) || formatLanguageName(normalizedCode)
      }
    })
...
export const getSupportedLanguageCodes = () => Array.from(supportedLanguageCodes)
