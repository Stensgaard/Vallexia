<template>
  <div class="space-y-1">
    ...
    <div
      v-if="showPercentage && percentage !== null"
      class="text-sm text-gray-600"
    >
      {{ settingsStore.formatNumberFn(percentage, 1)
      }}{{ $t("profile.nutritional.percentageOfCalories") }}
    </div>
  </div>
</template>
...
const props = defineProps({
  ...
  macroType: {
    ...
    default: null,
    validator: (value) =>
      !value || ["protein", "carbs", "fats"].includes(value),
  },
});
...
const percentage = computed(() => {
  ...
  const calories =
    props.macroCalories ??
    inputValue.value * (props.macroType === "fats" ? 9 : 4);
